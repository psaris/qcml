...
/ theta:flip first each theta`last;
 theta}
predict:{[X;theta]sum (((1;count X 0)#1f),X)*theta} / regression predict
lpredict:(')[sigmoid;predict]    / logistic regression predict
wmax:first idesc@                / where max?
/ predict each number and pick best
predictonevsall:{[X;THETA]wmax each flip lpredict[X]'[THETA]}
...
100*avg y=1+predictonevsall[X] THETA / what percent did we get correct?
